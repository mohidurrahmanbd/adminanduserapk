...
const isYearJobSolutionVariation = (name: string): boolean => {
  ...
  return (
    ...
    normalized === 'সাল ভিত্তিক জব সল্য্শন' ||
    normalized === 'সালভিত্তিক জব সল্য্শন' ||
...
const isSubjectPrepRootOrSubject = (name: string): boolean => {
  ...
  return (
    ...
    norm === 'নৈতিকতা ও ম্ল্যবোধ' ||
    norm === 'নৈতিকতা, মূল্যবোধ ও স্শাসন' ||
    norm === 'স্শাসন'
  );
};
...
const renderSubjectIcon = (name: string, sizeClass = "w-4 h-4") => {
  ...
  if (norm.includes('বাংলাদেশ') || norm.includes('bangladesh') || norm.includes('সংবিধান') || norm.includes('ম্ক্তিয্দ্ধ')) {
  ...
  if (norm.includes('ভূগোল') || norm.includes('পরিবেশ') || norm.includes('দ্র্যোগ') || norm.includes('geography')) {
    return <Compass className={sizeClass} />;
  }
  if (norm.includes('মানসিক দক্ষতা') || norm.includes('ব্দ্ধিমত্তা') || norm.includes('mental')) {
    return <BrainCircuit className={sizeClass} />;
  }
  if (norm.includes('নৈতিকতা') || norm.includes('মূল্যবোধ') || norm.includes('স্শাসন') || norm.includes('ethics')) {
  ...
  if (norm.includes('প্রাথমিক') || norm.includes('প্রাইমারি') || norm.includes('শিক্ষক') || norm.includes('স্ক্ল')) {
    return <School className={sizeClass} />;
  }
  if (norm.includes('ইতিহাস') || norm.includes('history') || norm.includes('এতিহ্য')) {
...
const getSubjectTheme = (name: string) => {
  ...
  if (norm.includes('নৈতিকতা') || norm.includes('মূল্যবোধ') || norm.includes('স্শাসন')) {
    return {
      bg: 'bg-indigo-600',
...
export default function UserPortal({
  ...
  const handleToggleEnrollCourse = (courseId: string, courseTitle: string) => {
    ...
    if (enrolledCourseIds.includes(courseId)) {
      showCustomConfirm(
        `"${courseTitle}" কোর্সটি থেকে কি আন-এনরোল করতে চান?`,
        () => {
  ...
  const handleEnrollSuccess = (enrollmentData: Omit<CourseEnrollment, 'id' | 'enrolledAt'>) => {
    ...
    const discountNotice = enrollmentData.discountAmount > 0 
      ? ` (${enrollmentData.discountPercent}% ক্পন ছাড় সহ ৳${enrollmentData.finalPrice} পরিশোধিত)` 
      : '';
    showCustomAlert('অভিনন্দন! 🎉', `"${enrollmentData.courseTitle}" কোর্সে আপনি সফলভাবে এনরোল করেছেন!${discountNotice} এটি "আমার কোর্স" সেকশনে যুক্ত হয়েছে।`, 'success');
    setSelectedCourseFilter('enrolled');
  };
  ...
  const getRoutineBatchInfo = (item: Routine) => {
    ...
    if (!hasCourse) {
      return {
        ...
        shortLabel: 'উন্ম্ক্ত (Open)',
        badgeClass: 'bg-blue-50 text-blue-800 border-blue-200',
        cardBorder: 'border-blue-200/80 hover:border-blue-300',
        description: 'উন্ম্ক্ত পরীক্ষা ও রুটিন — কোনো কোর্সে এনরোলমেন্ট ছাড়াই সরাসরি অংশগ্রহণযোগ্য।',
  ...
  const checkCourseEnrollmentAccess = (routine: Routine, featureName: string = 'পরীক্ষা ও প্রস্তুতি'): boolean => {
    ...
    if (batchInfo.type === 'unrolled') {
      ...
      showCustomAlert(
        `🔒 কোর্সে এনরোল প্রয়োজন!\n\n"${courseTitle}" কোর্সের ${featureName} শুধুমাত্র এনরোল্ড শিক্ষার্থীদের জন্য উন্ম্ক্ত।\n\nআপনি বর্তমানে শুধুমাত্র রুটিন ও সিলেবাস দেখতে পারবেন। ${featureName} আনলক করতে অন্গ্রহ করে কোর্সে এনরোল (Enroll) করুন।`,
        () => {
          if (courseId) {
  ...
  const handleOpenRoutinePreparation = (routine: Routine) => {
    if (!checkCourseEnrollmentAccess(routine, 'পরীক্ষার প্রস্তুতি (Chapter MCQ)')) {
      return;
    }
  ...
  const startDemoExam = async (routine: Routine) => {
    ...
    if (finalQuestions.length === 0) {
      showCustomAlert('দ্ঃখিত, ডেমো পরীক্ষার জন্য কোনো প্রশ্ন পাওয়া যায়নি!');
      return;
    }
  ...
  const formatExamScheduleWithTime = (dateStr?: string) => {
    ...
    try {
      ...
      const months = [
        'জান্য়ারি', 'ফেব্র্য়ারি', 'মার্চ', 'এপ্রিল', 'মে', 'জ্ন',
        'জ্লাই', 'আগস্ট', 'সেপ্টেম্বর', 'অক্টোবর', 'নভেম্বর', 'ডিসেম্বর'
      ];
      ...
      let period = 'সকাল';
      if (hours >= 12 && hours < 15) period = 'দ্প্র';
  ...
  const courseRoutineGroups = useMemo(() => {
    ...
    if (unassignedRoutines.length > 0) {
      groupMap.set('open_general_course', {
        ...
        courseTitle: 'উন্ম্ক্ত / সাধারণ রুটিন (Open Batch)',
        category: 'উন্ম্ক্ত প্রস্তুতি',
    ...
    result.forEach(g => {
      ...
      g.nextExamDateStr = nextExamTimeStr || fallbackDateStr || 'শিডিউল অন্যায়ী';
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="summary-grid">
          ...
          <div class="summary-card">
            <div class="summary-label">অন্ত্তর / স্কিপড</div>
        ...
        ${includeMarkCalcTable ? `
          <div class="mark-calc-box">
            <div class="mark-calc-title">📚 বিষয়ভিত্তিক নম্বর ও সঠিক-ভুল বিবরণী</div>
            <table class="mark-calc-table">
              <thead>
                <tr>
                  <th style="text-align: left;">বিষয়</th>
                  <th>মোট প্রশ্ন</th>
                  <th style="color: #166534;">সঠিক</th>
                  <th style="color: #9f1239;">ভুল</th>
                  <th style="color: #92400e;">স্কিপড</th>
                  <th style="text-align: right;">মোট নম্বর</th>
                </tr>
              </thead>
              <tbody>
                ...
                `).join('')}
                <tr class="total-row">
                  <td style="text-align: left; font-weight: 800; color: #1e1b4b;">🏆 সর্বমোট</td>
  ...
  const parseAnyDate = (dateStr?: string): Date | null => {
    ...
    const bnMonths: Record<string, number> = {
      'জান্য়ারি': 0, 'জান্য়ারি': 0, 'january': 0, 'jan': 0,
      'ফেব্র্য়ারি': 1, 'ফেব্র্য়ারি': 1, 'february': 1, 'feb': 1,
      'মার্চ': 2, 'march': 2, 'mar': 2,
      'এপ্রিল': 3, 'april': 3, 'apr': 3,
      'মে': 4, 'may': 4,
      'জ্ন': 5, 'june': 5, 'jun': 5,
      'জ্লাই': 6, 'july': 6, 'jul': 6,
  ...
  const getBengaliRelativeTime = (dateObj: Date): string => {
    ...
    if (diffDays === 0) return 'আজ অন্ষ্ঠিত';
  ...
  // â”€â”€â”€ YEAR-WISE JOB SOLUTION DYNAMIC AUTO-MAPPING (UI-ONLY HIERARCHY) â”€â”€â”€
  ...
  const yearWiseJobSolutionTree = useMemo(() => {
    ...
    candidateExamNodesMap.forEach(node => {
      ...
      const cleanName = node.name.replace(/\s*\([\d\s\u09E6-\u09EF\u0980-\u09FF\w\/\.-]+(?:\s*(?:জান্য়ারি|ফেব্র্য়ারি|মার্চ|এপ্রিল|মে|জ্ন|জ্লাই|আগস্ট|সেপ্টেম্বর|অক্টোবর|নভেম্বর|ডিসেম্বর|january|february|march|april|may|june|july|august|september|october|november|december)\s*)?[\d\s\u09E6-\u09EF\w]*\)\s*$/i, '').trim() || node.name;
  ...
  const startPrepExam = async (categoryName: string, overrideQuestions?: Question[]) => {
    ...
    if (onFetchQuestionsLazy && !overrideQuestions) {
      const fetched = await onFetchQuestionsLazy({ category: categoryName });
      if (fetched && fetched.length > 0) {
        const fetchedFiltered = fetched.filter(q => q.category === categoryName || (q.categories && q.categories.includes(categoryName)));
        if (fetchedFiltered.length > 0) {
          const map = new Map<string, Question>();
          filtered.forEach(q => map.set(String(q.id), q));
          fetchedFiltered.forEach(q => map.set(String(q.id), q));
  ...
  const startJobExam = async (subcatName: string, overrideQuestions?: Question[]) => {
    ...
    if (onFetchQuestionsLazy && !overrideQuestions) {
      ...
      if (fetched && fetched.length > 0) {
        const fetchedFiltered = fetched.filter(q => q.subcategory === subcatName || (q.subcategories && q.subcategories.includes(subcatName)));
        if (fetchedFiltered.length > 0) {
          const map = new Map<string, Question>();
          filtered.forEach(q => map.set(String(q.id), q));
          fetchedFiltered.forEach(q => map.set(String(q.id), q));
  ...
  const startOfficialLiveExam = async (exam: LiveExam) => {
    ...
    if (exam.questionIds && exam.questionIds.length > 0) {
      ...
      if (pool.length === 0) {
        showCustomAlert('দ্ঃখিত, এই পরীক্ষার সাথে সম্পর্কিত কোনো কুইজ ডাটাবেসে পাওয়া যায়নি!');
        return;
      }
    ...
    if (overrideQuestions) {
      ...
      if (type === 'prep') {
        ...
        if (onFetchQuestionsLazy) {
          ...
          if (fetched && fetched.length > 0) {
            const fetchedFiltered = fetched.filter(q => q.category === targetValue || (q.categories && q.categories.includes(targetValue)));
            if (fetchedFiltered.length > 0) {
              const map = new Map<string, Question>();
              filtered.forEach(q => map.set(String(q.id), q));
              fetchedFiltered.forEach(q => map.set(String(q.id), q));
        ...
        if (onFetchQuestionsLazy) {
          ...
          if (fetched && fetched.length > 0) {
            const fetchedFiltered = fetched.filter(q => q.subcategory === targetValue || (q.subcategories && q.subcategories.includes(targetValue)));
            if (fetchedFiltered.length > 0) {
              const map = new Map<string, Question>();
              filtered.forEach(q => map.set(String(q.id), q));
              fetchedFiltered.forEach(q => map.set(String(q.id), q));
  ...
  const renderModals = () => {
    return (
      <>
                ...
                <div className="space-y-2">
                  ...
                  {(() => {
                    ...
                    syllabusPaths.forEach(path => {
                      ...
                      const root = parts[0];
                      const subHierarchy = parts.length > 1 ? parts.slice(1).join(" Ã¢â‚¬Âº ") : "";
                      if (!rootMap.has(root)) {
                        rootMap.set(root, []);
                      }
                            ...
                            {subList.length > 0 ? (
                              <div className="space-y-1.5 pl-1.5">
                                {subList.map((sub, sIdx) => {
                                  const subParts = sub.split(/\s*Ã¢â‚¬Âº\s*/);
                                  return (
                                    <div 
                                      key={`sub-${sIdx}`}
                                    ...
                                    >
                                      {subParts.map((seg, segIdx, arr) => (
                                        <React.Fragment key={segIdx}>
                                          ...
                                          {segIdx < arr.length - 1 && <span className="text-indigo-400 font-bold">Ã¢â‚¬Âº</span>}
                                        </React.Fragment>
                                      ))}
                                    </div>
                                  );
            ...
            >
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="ভুলটি কি এবং সঠিক তথ্য কি হবে তা বিস্তারিত লিখ্ন..."
          ...
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
            <form
              onSubmit={(ev) => {
                ...
                if (!allowUserExplanation) {
                  alert('দ্ঃখিত, বর্তমানে ব্যাখ্যা যোগ করার স্বিধাটি সাময়িকভাবে বন্ধ রয়েছে।');
                  return;
                }
                if (onUpdateQuestion) {
                  const newExpl = {
                  ...
                  alert('আপনার ব্যাখ্যাটি সফলভাবে প্রেরণ করা হয়েছে! এডমিন প্যানেল থেকে অন্মোদন মিললে এটি প্রকাশ করা হবে।');
                  setUserExplModalQ(null);
                  setUserExplText('');
                }
              }}
            ...
            >
              ...
              <div className="flex items-center gap-2 text-amber-700 font-bold">
                <span>✍︍ সঠিক উত্তরের ব্যাখ্যা প্রদান করুন</span>
              </div>
              ...
              <div>
                <label className="block text-gray-700 font-bold mb-1.5">
                  প্রশ্নের উত্তরের সঠিক ব্যাখ্যা লিখ্ন:
                </label>
                <textarea
                  required
                  rows={4}
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10.5px] text-slate-500 font-medium">
                      প্রাপ্ত নম্বর দিয়ে ফেসব্কে বন্ধ্দের সরাসরি চ্যালেঞ্জ জানান!
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="flex flex-col gap-2 pt-2 border-t border-gray-100">
                <button
                  ...
                  onClick={() => {
                    ...
                    showCustomAlert(
                      `আপনার নম্বর (${challengeModalData.score}) সহ চ্যালেঞ্জ মেসেজ ক্লিপবোর্ডে কপি করা হয়েছে এবং ফেসব্ক শেয়ার পেজ চাল্ হয়েছে!\n\nফেসব্কে পেস্ট (Paste) করে বন্ধ্দের চ্যালেঞ্জ পোস্ট করুন।`,
                      undefined,
                      '⚔︍ ফেসব্ক চ্যালেঞ্জ প্রস্ত্ত!'
                    );
                  }}
                ...
                >
                  <span>📘</span> ফেসব্কে চ্যালেঞ্জ শেয়ার করুন (FB Share)
                </button>
        ...
        {/* Course Enrollment & Discount Coupon Modal */}
        ...
        {bookmarkModalOpen && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl p-6 relative border border-gray-100 flex flex-col gap-4 animate-fade-in text-xs">
              <button
                type="button"
                onClick={() => {
                  setBookmarkModalOpen(false);
                ...
                className="absolute top-4 right-4 p-2 bg-gray-50 hover:bg-gray-100 text-gray-500 hover:text-gray-700 rounded-full transition cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>

              <div className="flex items-center gap-2 text-amber-700 font-bold text-sm">
                <BookmarkIcon className="w-4 h-4 fill-amber-500 text-amber-500" />
                <span>বুকমার্ক কালেকশনে সংরক্ষণ</span>
              </div>

              <p className="text-gray-600 font-medium text-xs">
                প্রশ্নটি কোন বুকমার্ক ফোল্ডারে রাখতে চান নির্বাচন করুন:
              </p>

              <div className="space-y-3">
                <div>
                  <label className="block text-gray-700 font-bold mb-1.5">ফোল্ডার নির্বাচন করুন:</label>
                  <select
                    value={isCustomFolder ? '__custom__' : bookmarkFolder}
                    onChange={(e) => {
                    ...
                    className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl font-bold text-xs text-gray-800 focus:outline-hidden focus:ring-2 focus:ring-amber-500"
                  >
                    <option value="সাধারণ বুকমার্ক">সাধারণ বুকমার্ক</option>
                    {Object.keys(groupedBookmarks)
                      .filter(f => f !== 'সাধারণ বুকমার্ক')
                      .map(folder => (
                        <option key={folder} value={folder}>{folder}</option>
                      ))}
                    <option value="__custom__">+ নতুন ফোল্ডার তৈরি করুন...</option>
                  </select>
                </div>

                {isCustomFolder && (
                  <div>
                    <label className="block text-gray-700 font-bold mb-1.5">নতুন ফোল্ডারের নাম:</label>
                    <input
                      type="text"
                      value={customFolderInput}
                      onChange={(e) => setCustomFolderInput(e.target.value)}
                      placeholder="যেমন: বিসিএস প্রস্তুতি, সাম্প্রতিক সাধারণ জ্ঞান..."
                      className="w-full p-2.5 bg-white border border-amber-300 rounded-xl font-bold text-xs text-gray-800 focus:outline-hidden focus:ring-2 focus:ring-amber-500"
                      autoFocus
                    />
                  </div>
                )}
              ...
              <div className="flex gap-2.5 pt-2 border-t border-gray-100 mt-2">
                <button
                  type="button"
                  onClick={() => {
                    setBookmarkModalOpen(false);
                ...
                <button
                  ...
                  className="flex-1 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs rounded-xl shadow-md transition cursor-pointer flex items-center justify-center gap-1.5"
                >
                  <BookmarkIcon className="w-3.5 h-3.5 fill-white" />
                  সংরক্ষণ করুন
                </button>
              </div>
            </div>
  ...
  return (
    <div className={`flex flex-col gap-3 min-h-[90vh] max-w-full overflow-x-hidden ${!quizActive && !readerModeActive ? 'pb-24' : ''}`}>
              ...
              <div className="flex-1 overflow-y-auto py-5 px-4 flex flex-col gap-6">
                ...
                <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 font-bold uppercase">মেন্ নেভিগেশন</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setDrawerOpen(false)}
                ...
                <div className="flex flex-col gap-1.5">
                  {[
                    ...
                    { id: 'exams', label: '⍱︍ এক্সাম জোন (পরীক্ষা)', icon: Clock },
          ...
          <div className="flex-grow flex flex-col bg-white border border-slate-200/90 shadow-xl rounded-2xl p-3.5 sm:p-5 md:p-6 animate-fade-in space-y-4">
              ...
              <div className="mt-3 pt-2.5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 p-1 bg-slate-100 rounded-xl border border-slate-200">
                  ...
                  >
                    ...
                    অন্ত্তর / বাকি ({toBengaliDigits(unansweredCount)})
                  </button>
                </div>
                      ...
                      {isSelected && quizAnswerMode === 'instant' && (() => {
                        ...
                        return (
                          <div className="flex flex-row flex-wrap gap-2 items-center pt-2.5 border-t border-slate-100">
                            ...
                            >
                              {allowUserExplanation ? '✍︍ ব্যাখ্যা +' : '✍︍ ব্যাখ্যা (বন্ধ)'}
                            </button>
                          </div>
                        );
                      })()}
            ...
            <div className="flex flex-col gap-3 pt-4 border-t border-slate-200">
              ...
              <div className="grid grid-cols-3 gap-2 bg-slate-50 p-3 rounded-2xl border border-slate-200 text-center">
                ...
                <div className="p-1">
                  <div className="text-[11px] text-rose-700 font-bold">অন্ত্তর / বাকি</div>
            ...
            <div className="my-3 flex-grow space-y-3">
              {paginatedQuestions.length === 0 ? (
                <div className="text-center py-6 text-gray-500 font-bold bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                  এই ক্যাটাগরিতে কোনো প্রশ্ন খ্এজে পাওয়া যায়নি।
                </div>
              ) : (
          ...
          {activeTab === 'dashboard' && (
            <div className="flex flex-col gap-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-indigo-700/80 mt-0.5">ইচ্ছামত পরীক্ষা তৈরি করুন</p>
                    </div>
                  </div>
              ...
              <div className="bg-white border border-gray-100 p-3 sm:p-3.5 rounded-2xl shadow-sm flex flex-col">
                ...
                <p className="text-[10px] text-gray-400 mb-2 font-semibold">বিগত পরীক্ষাগ্লোর ফলাফলের ওপর ভিত্তি করে পরিসংখ্যান</p>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {exam.formattedDate ? (
                    <div className="flex items-center gap-1 text-[10.5px] font-bold text-emerald-700 mt-0.5">
                      ...
                      <span>অন্ষ্ঠিত: {exam.formattedDate}</span>
                    </div>
                  ) : (
                        ...
                        if (onFetchQuestionsLazy) {
                          ...
                          if (fetched && fetched.length > 0) {
                            ...
                            if (fetchedFiltered.length > 0) {
                              ...
                              fetchedFiltered.forEach(q => map.set(String(q.id), q));
                              examQuestions = Array.from(map.values());
                            }
                          }
                        }
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (onFetchQuestionsLazy) {
                          const fetched = await onFetchQuestionsLazy({ subcategory: exam.name });
                          if (fetched && fetched.length > 0) {
                            const fetchedFiltered = fetched.filter(q => q.subcategory === exam.name || (q.subcategories && q.subcategories.includes(exam.name)));
                            if (fetchedFiltered.length > 0) {
                              const map = new Map<string, Question>();
                              pool.forEach(q => map.set(String(q.id), q));
                              fetchedFiltered.forEach(q => map.set(String(q.id), q));
                        ...
                        if (pool.length === 0) {
                          showCustomAlert('দ্ঃখিত, এই পরীক্ষার সাথে সম্পর্কিত কোনো প্রশ্ন পাওয়া যায়নি!');
                          return;
                        }
            ...
            return (
              <div className="bg-white border border-slate-200/80 p-3 sm:p-4 rounded-2xl shadow-2xs flex flex-col gap-4 text-xs animate-fade-in">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-3 border-b border-slate-100">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      title={selectedRecentJobMonth ? "বিগত সকল মাসের তালিকায় ফির্ন" : "ড্যাশবোর্ডে ফিরে যান"}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-semibold mt-0.5">
                        {selectedRecentJobMonth 
                          ? `${selectedRecentJobMonth} মাসে অন্ষ্ঠিত সকল সরকারি ও নিয়োগ পরীক্ষার প্রশ্ন সমাধান` 
                          : 'চলতি মাসের অন্ষ্ঠিত পরীক্ষা এবং বিগত মাসভিত্তিক প্রশ্নব্যাংক'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="relative w-full sm:w-64 shrink-0">
                    ...
                    <input
                      ...
                      placeholder="পরীক্ষার নাম বা মাস দিয়ে খ্এজ্ন..."
                ...
                {isSearching ? (
                  ...
                  /* CASE 2: A SPECIFIC PREVIOUS MONTH IS SELECTED (e.g. জ্লাই ২০২৬) */
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex items-center justify-between pb-1 flex-wrap gap-2">
                      ...
                      >
                        ...
                        <span>বিগত সকল মাসের তালিকায় ফির্ন</span>
                      </button>
                    ...
                    <div className="flex flex-col gap-2.5">
                      <div className="flex items-center justify-between flex-wrap gap-2 pb-1 border-b border-slate-100">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-xs sm:text-sm font-extrabold text-slate-800 flex items-center gap-1.5">
                            চলতি মাসে অন্ষ্ঠিত পরীক্ষা ({currentRunningMonthName})
                          </h3>
                        </div>
                      ...
                      {runningMonthExams.length === 0 ? (
                        <div className="p-3.5 bg-emerald-50/40 rounded-xl border border-emerald-100 text-emerald-800 text-[11px] font-semibold flex items-center gap-2">
                          ...
                          <span>চলতি মাসে ({currentRunningMonthName}) এখনো নতুন কোনো পরীক্ষা অন্ষ্ঠিত বা যোগ হয়নি।</span>
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                    ...
                    <div className="flex flex-col gap-2.5 pt-2 border-t border-slate-100">
                      ...
                      {previousMonthGroups.length === 0 ? (
                        <div className="p-4 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-center text-slate-400 text-xs">
                          বিগত ৫ মাসে অন্ষ্ঠিত কোনো পরীক্ষার রেকর্ড পাওয়া যায়নি।
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                                    ...
                                    onClick={async () => {
                                      ...
                                      if (onFetchQuestionsLazy) {
                                        ...
                                        if (fetched && fetched.length > 0) {
                                          ...
                                          if (fetchedFiltered.length > 0) {
                                            const map = new Map<string, Question>();
                                            subcatQuestions.forEach(q => map.set(String(q.id), q));
                                            fetchedFiltered.forEach(q => map.set(String(q.id), q));
                                            subcatQuestions = Array.from(map.values());
                                          }
                                        }
                                      }
                                  ...
                                  >
                                    ...
                                    <div className="pt-1 border-t border-slate-100 flex items-center justify-between text-[8.5px]">
                                      ...
                                      <span className="text-emerald-700 font-bold flex items-center gap-0.5 group-hover:translate-x-0.5 transition-transform">
                                        খ্ল্ন <ChevronRight className="w-2.5 h-2.5" />
                                      </span>
                                    </div>
                                  </button>
                                );
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-2.5 border-b border-slate-100">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10.5px] text-slate-500 font-medium">
                        পরীক্ষার সাল ও মাস অন্যায়ী সাজানো প্রশ্ন সমাধান ব্যাংক (সর্বশেষ পরীক্ষা সবার উপরে)
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap sm:flex-nowrap">
                    ...
                    <div className="relative flex-1 sm:w-56">
                      ...
                      <input
                        ...
                        placeholder="পরীক্ষা বা সাল খ্এজ্ন..."
                    ...
                    <button
                      ...
                      title={expandedYearKeys.size > 0 ? "সব সংক্চিত করুন" : "সব সম্প্রসারণ করুন"}
                    >
                ...
                {filteredYears.length === 0 ? (
                  <div className="text-center text-gray-500 py-10 bg-slate-50/70 rounded-xl border border-dashed border-slate-200 flex flex-col items-center justify-center gap-2 p-4">
                    ...
                    <span className="text-[11px] text-slate-400 max-w-sm">
                      জব সলিউশন ব্যাংকে নতুন পরীক্ষা তৈরি করা হলে তা স্বয়ংক্রিয়ভাবে তার পরীক্ষার সাল ও মাস অন্যায়ী এখানে প্রদর্শিত হবে।
                    </span>
                  </div>
                ) : (
                  <div className="flex flex-col gap-2.5">
                            ...
                            <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                              ...
                              <div className="flex items-center gap-2 flex-wrap min-w-0">
                                <span className="font-black text-[13px] sm:text-[14.5px] text-slate-900 leading-tight">
                                  {isYearExpanded ? 'â–¼ ' : '▶ '} {yearGroup.yearLabel}
                                </span>
                              </div>
                            </div>
                                    ...
                                    >
                                      <div className="flex items-center gap-2 min-w-0">
                                        ...
                                        <span className="font-extrabold text-[12.5px] sm:text-[13.5px] text-slate-800">
                                          {isMonthExpanded ? 'â–¼ ' : '▶ '} {monthGroup.monthLabel}
                                        </span>
                                      </div>
                          ...
                          <div className="flex items-center gap-1 sm:gap-1.5 flex-nowrap shrink-0">
                            ...
                            <button
                              onClick={() => {
                                ...
                                showCustomAlert('লাইভ পরীক্ষার লিঙ্ক ক্লিপবোর্ডে কপি করা হয়েছে!\nএখন যেকোনো বন্ধ্দের সাথে বা সোশ্যাল মিডিয়ায় শেয়ার করতে পারবেন।', undefined, 'কপি সম্পন্ন!');
                              }}
                            ...
                            {alreadyTaken && (
                              <button
                                ...
                                title="আপনার প্রাপ্ত নম্বর দিয়ে বন্ধ্দের ফেসব্কে চ্যালেঞ্জ পাঠান"
                              >
                                <span>⚔︍</span> <span className="whitespace-nowrap">চ্যালেঞ্জ</span>
                              </button>
                            )}
                          </div>
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-extrabold text-indigo-950 text-sm sm:text-base flex items-center gap-1.5">
                          🛡︍ {selectedAttemptForView.examTitle}
                        </h3>
                        {attemptCourseName && (
                      ...
                      <div className="bg-white border border-slate-200/90 rounded-2xl p-4 sm:p-5 shadow-2xs space-y-3">
                        <div className="flex flex-wrap items-center justify-between gap-2 pb-2.5 border-b border-slate-100">
                          <h4 className="font-extrabold text-slate-900 text-xs sm:text-sm flex items-center gap-2">
                            <span>📚 বিষয়ভিত্তিক নম্বর ও সঠিক-ভুল বিবরণী</span>
                          </h4>
                          ...
                          <table className="w-full text-left text-xs border-collapse">
                            <thead>
                              <tr className="bg-slate-50 text-slate-700 border-b border-slate-200 font-bold">
                                <th className="p-2.5 rounded-l-xl">বিষয়</th>
                                <th className="p-2.5 text-center">মোট প্রশ্ন</th>
                                <th className="p-2.5 text-center text-emerald-700">সঠিক</th>
                                <th className="p-2.5 text-center text-rose-700">ভুল</th>
                                <th className="p-2.5 text-center text-amber-700">স্কিপড</th>
                                <th className="p-2.5 text-right rounded-r-xl">মোট নম্বর</th>
                              </tr>
                            </thead>
                          ...
                          <div key={q.id || i} className="pt-3 text-xs flex flex-col gap-2">
                            ...
                            {masterQ.userExplanations?.filter(e => e.approved).map((e) => (
                              <div key={e.id} className="p-2 rounded-xl bg-amber-50/80 border border-amber-100 text-amber-900 text-[10px] leading-relaxed">
                                <span className="font-bold text-amber-950">✍︍ {e.userName}: </span>
                                {e.text}
                              </div>
                            ))}
                          </div>
                ...
                <div className="bg-white border border-gray-100 py-3 px-1 sm:px-4 sm:py-4 rounded-3xl shadow-sm flex flex-col gap-3 sm:gap-4 text-xs animate-fade-in">
                  <div className="flex flex-col gap-3 sm:gap-4">
                    ...
                    <div className="flex bg-slate-100 p-1 sm:p-1.5 rounded-2xl gap-1 sm:gap-1.5 border border-slate-200/40">
                      ...
                      >
                        🛡︍ অফিশিয়াল লাইভ পরীক্ষা ({adminCreatedAttempts.length})
                      </button>
                    </div>
                    ...
                    {activeFilteredAttempts.length === 0 ? (
                      <p className="text-center py-10 text-gray-400 font-bold">
                        {resultFilterMode === 'user' 
                          ? 'আপনার কোনো কাস্টম বা অধ্যায়ভিত্তিক পরীক্ষার রেকর্ড পাওয়া যায়নি (রেকর্ডসমূহ ৭২ ঘণ্টা পর স্বয়ংক্রিয়ভাবে ম্ছে যায়)।'
                                ...
                                <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                                  ...
                                  {isUserCreated && (
                                    <span className="text-[8.5px] sm:text-[9px] font-black text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded-md border border-rose-100 shrink-0">
                                      ⍱︍ আর {Math.ceil(hoursLeft).toLocaleString('bn-BD')} ঘণ্টা সংরক্ষিত
                                    </span>
                                  )}
                                  {!isUserCreated && (() => {
              ...
              {(() => {
                ...
                if (expandedCourseId && selectedCourse) {
                  ...
                  return (
                    <div className="space-y-5 animate-fade-in">
                      ...
                      <div className={`bg-white border ${isEnrolled ? 'border-emerald-300 ring-2 ring-emerald-500/10' : 'border-gray-100/90'} p-3.5 sm:p-5 rounded-2xl shadow-sm space-y-3.5 relative overflow-hidden`}>
                        {isEnrolled && (
                          <div className="absolute top-3 right-3 z-10 bg-emerald-600 text-white font-black text-[10px] sm:text-[11px] uppercase tracking-wider px-2.5 py-0.5 rounded-lg shadow-xs flex items-center gap-1 border border-emerald-500/90 pointer-events-none">
                        ...
                        {/* Status Badges */}
                        <div className={`flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 pb-2.5 ${isEnrolled ? 'pr-24' : ''}`}>
                          <div className="flex flex-wrap items-center gap-1.5">
                            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-xl text-[10px] font-black uppercase tracking-wider ${
                        ...
                        {/* Title & Description */}
                        <div>
                          <h2 className="font-extrabold text-indigo-950 text-base sm:text-lg leading-snug">
                            {selectedCourse.title}
                          </h2>
                          <p className="text-gray-600 font-medium text-xs mt-1.5 leading-relaxed whitespace-pre-line">
                            {selectedCourse.description}
                          </p>
                        </div>

                        {/* Dates, Price & Metrics */}
                        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-[9px] sm:text-[10.5px] font-semibold text-gray-600 pt-0.5">
                          {/* Price Tag (hidden after enrolling) */}
                          {!isEnrolled && (
                            <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/80 px-2.5 py-1 rounded-xl flex items-center gap-1.5 shadow-2xs">
                              <span className="text-amber-800 font-extrabold">ফি:</span>
                              {selectedCourse.price && selectedCourse.price > 0 ? (
                                <div className="flex items-center gap-1">
                                  <span className="font-black text-amber-900 text-xs">৳{selectedCourse.price}</span>
                                  {selectedCourse.originalPrice && selectedCourse.originalPrice > selectedCourse.price && (
                                    <span className="line-through text-slate-400 text-[10px]">৳{selectedCourse.originalPrice}</span>
                                  )}
                                </div>
                              ) : (
                                <span className="font-black text-emerald-700">বিনামূল্যে (Free)</span>
                              )}
                            </div>
                          )}

                          {selectedCourse.startDate && (
                            <span className="flex-1 sm:flex-initial min-w-0 bg-slate-50 border border-slate-100 px-1.5 sm:px-2.5 py-1 rounded-xl flex items-center justify-center text-slate-800 font-bold whitespace-nowrap text-[9px] sm:text-[10.5px]">
                          ...
                          <span className="flex-1 sm:flex-initial min-w-0 bg-indigo-50 border border-indigo-100 px-1.5 sm:px-2.5 py-1 rounded-xl text-indigo-800 font-bold flex items-center justify-center whitespace-nowrap text-[9px] sm:text-[10.5px]">
                            <span className="truncate">মোট পরীক্ষা: {toBengaliDigits(courseRoutines.length)}টি</span>
                          </span>
                        </div>
                            ...
                            <div className="flex items-center gap-2">
                              {coupons && coupons.some(c => c.isActive) && (
                                <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200/80 px-2.5 py-1 rounded-xl">
                                  ...
                                  ডিসকাউন্ট ক্পন কোড ব্যবহার করে ছাড় পান!
                                </span>
                              )}
                            </div>
                      ...
                      <div className="space-y-3">
                        ...
                        {courseRoutines.length === 0 ? (
                          <div className="p-6 bg-white rounded-2xl border border-gray-100 text-center space-y-2">
                          ...
                          <div className="space-y-2.5">
                            {courseRoutines.map((r, rIdx) => {
                              ...
                              return (
                                <div key={r.id || rIdx} className="p-3 sm:p-3.5 bg-white border border-gray-200/80 rounded-xl text-xs space-y-2.5 shadow-2xs">
                                      ...
                                      subList.forEach((sub, sIdx) => {
                                        displayLines.push(
                                          <div key={`${root}-sub-${sIdx}`} className="text-black font-normal text-xs pl-2.5 flex items-start gap-1 leading-snug">
                                            <span className="text-black font-bold select-none">Ã¢â‚¬Âº</span>
                                      ...
                                      return (
                                        <>
                                          ...
                                          >
                                            ...
                                            <span className="truncate">পরীক্ষার প্রস্তুতি</span>
                                          </button>
                                          <button
                                            ...
                                            hasExam && (
                                              isExamLive ? (
                                                ...
                                                <span className="flex-1 min-w-0 bg-amber-100 text-amber-900 border border-amber-200 px-1 sm:px-3 py-2 rounded-xl text-[10px] sm:text-xs font-extrabold whitespace-nowrap text-center flex items-center justify-center">
                                                  <span className="truncate">পরীক্ষা শুরু হয়নি</span>
                                                </span>
                                              )
                                            )
                                          )}
                ...
                return (
                  <div className="space-y-5">
                    {/* Header Banner */}
                    <div className="bg-gradient-to-r from-indigo-900 via-slate-900 to-purple-900 text-white px-3.5 sm:px-4.5 py-3.5 rounded-2xl shadow-md border border-indigo-700/40 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3.5">
                      <div>
                        <span className="inline-block px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-indigo-500/30 text-indigo-200 border border-indigo-400/30 mb-1">
                          🎓 একাডেমি ও জব সলিউশন
                        </span>
                        <h2 className="text-base sm:text-lg font-extrabold text-white flex items-center gap-2">
                          <GraduationCap className="w-5 h-5 text-indigo-300" />
                          আমার কোর্স ও স্টাডি পুরোগ্রাম
                        </h2>
                      ...
                      {/* Filter Controls */}
                      <div className="flex flex-wrap gap-1 bg-white/10 p-1 rounded-xl border border-white/15 backdrop-blur-sm self-stretch sm:self-auto">
                    ...
                    {filteredCourses.length === 0 ? (
                      <div className="bg-white border border-gray-100 px-4 py-8 sm:py-10 rounded-2xl text-center space-y-3 shadow-sm">
                        <div className="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center text-2xl mx-auto font-bold shadow-inner">
                          🎓
                        </div>
                        ...
                        <p className="text-gray-500 max-w-md mx-auto text-xs leading-relaxed font-medium">
                          {selectedCourseFilter === 'enrolled' 
                            ? 'আমাদের উপলব্ধ কোর্সগ্লো থেকে আপনার পছন্দের কোর্সে এনরোল করুন। এনরোলকৃত কোর্সসমূহ সরাসরি এখানে জমা থাকবে।' 
                            : 'নির্বাচিত ফিল্টারে কোনো কোর্স পাওয়া যায়নি। অন্য ফিল্টার নির্বাচন করুন।'}
                        </p>
                        {selectedCourseFilter === 'enrolled' && (
                          <button
                            onClick={() => setSelectedCourseFilter('all')}
                            className="mt-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold px-4 py-2 rounded-xl transition shadow text-xs inline-flex items-center gap-2"
                          >
                        ...
                        {filteredCourses.map((course, idx) => {
                          ...
                          return (
                            <div 
                              ...
                              className={`bg-white border ${isEnrolled ? 'border-emerald-300 ring-2 ring-emerald-500/10' : 'border-gray-100/90 hover:border-indigo-300'} p-3.5 sm:p-4 rounded-2xl shadow-sm hover:shadow-md transition flex flex-col justify-between gap-3.5 relative overflow-hidden cursor-pointer group`}
                            >
                              {isEnrolled && (
                                <div className="absolute top-2.5 right-2.5 z-10 bg-emerald-600 text-white font-black text-[10px] uppercase tracking-wider px-2.5 py-0.5 rounded-lg shadow-xs flex items-center gap-1 border border-emerald-500/90 pointer-events-none">
                                  <Check className="w-3 h-3 stroke-[3]" />
                                  <span>Enrolled</span>
                                </div>
                              )}
                              ...
                              <div className="space-y-2.5">
                                {/* Course Badges */}
                                <div className={`flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 pb-2 ${isEnrolled ? 'pr-24' : ''}`}>
                                  <div className="flex flex-wrap items-center gap-1.5">
                                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-xl text-[10px] font-black uppercase tracking-wider ${
                                ...
                                <div>
                                  ...
                                  <p className="text-gray-600 font-medium text-xs mt-1 leading-relaxed line-clamp-2">
                                    {course.description}
                                  </p>
                                </div>

                                {/* Price & Coupon Badge (hidden after enrolling) */}
                                {!isEnrolled && (
                                  <div className="flex flex-wrap items-center justify-between gap-1.5 bg-slate-50 border border-slate-200/70 p-2 rounded-xl text-xs">
                                    <div className="flex items-center gap-1.5">
                                      <span className="text-slate-500 font-bold text-[11px]">ফি:</span>
                                      {course.price && course.price > 0 ? (
                                        <div className="flex items-center gap-1.5">
                                          <span className="font-black text-indigo-950 text-xs sm:text-sm">৳{course.price}</span>
                                          {course.originalPrice && course.originalPrice > course.price && (
                                            <>
                                              <span className="line-through text-slate-400 text-[10.5px]">৳{course.originalPrice}</span>
                                              <span className="text-[9.5px] bg-rose-100 text-rose-800 font-black px-1.5 py-0.2 rounded border border-rose-200">
                                                {Math.round(((course.originalPrice - course.price) / course.originalPrice) * 100)}% ছাড়
                                              </span>
                                            </>
                                          )}
                                        </div>
                                      ) : (
                                        <span className="font-black text-emerald-700 text-xs">বিনামূল্যে (Free)</span>
                                      )}
                                    </div>

                                    {coupons && coupons.some(c => c.isActive) && (
                                      <span className="inline-flex items-center gap-1 text-[10px] font-black text-emerald-700 bg-emerald-100/70 px-2 py-0.5 rounded-lg border border-emerald-200/80">
                                ...
                                {/* Key Metadata without icons */}
                                <div className="flex flex-nowrap items-center gap-1 sm:gap-1.5 text-[9px] sm:text-[10px] font-semibold text-gray-500 pt-0.5">
                                  {course.startDate && (
                                    <span className="flex-1 min-w-0 bg-gray-50 border border-gray-100 px-1 sm:px-2 py-1 rounded-xl flex items-center justify-center text-gray-700 font-bold whitespace-nowrap text-[9px] sm:text-[10px]">
                                      <span className="truncate">শুরু: {course.startDate}</span>
                                    </span>
                                  )}
                                  {course.endDate && (
                                    <span className="flex-1 min-w-0 bg-gray-50 border border-gray-100 px-1 sm:px-2 py-1 rounded-xl flex items-center justify-center text-gray-700 font-bold whitespace-nowrap text-[9px] sm:text-[10px]">
                                      <span className="truncate">শেষ: {course.endDate}</span>
                                    </span>
                                  )}
                                  <span className="flex-1 min-w-0 bg-indigo-50 border border-indigo-100 px-1 sm:px-2 py-1 rounded-xl text-indigo-800 font-bold flex items-center justify-center whitespace-nowrap text-[9px] sm:text-[10px]">
                                    <span className="truncate">মোট পরীক্ষা: {toBengaliDigits(courseRoutines.length)} টি</span>
                                  </span>
                                </div>

                                {/* Click Prompt Bar */}
                                <div className="bg-slate-50 group-hover:bg-indigo-50/80 border border-slate-200/80 px-2.5 py-2 rounded-xl flex justify-between items-center transition">
                              ...
                              {/* Action Buttons */}
                              <div className="flex flex-wrap items-center gap-2 pt-2.5 border-t border-gray-100" onClick={(e) => e.stopPropagation()}>
                                <button
                                  onClick={() => downloadCourseRoutinePDF(course.title, course.category, courseRoutines, subcategories, categories, questions)}
                                  className="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-extrabold py-2 px-3 rounded-xl transition text-xs flex items-center justify-center gap-1 border border-indigo-200/80 shadow-2xs"
                                ...
                                {isEnrolled ? (
                                  <div className="flex items-center gap-2 flex-1 justify-end">
                                    <span className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-extrabold bg-emerald-100 text-emerald-800 border border-emerald-200 shadow-2xs">
                                  ...
                                  <button
                                    ...
                                    className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold py-2 px-3.5 rounded-xl transition text-center shadow flex items-center justify-center gap-1.5 text-xs"
                                  >
                                    <GraduationCap className="w-3.5 h-3.5 text-indigo-200" />
                                    <span>এনরোল করুন {course.price ? `(৳${course.price})` : '(ফ্রি)'}</span>
                                  </button>
                                )}
                              </div>
                            </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 bg-slate-50 p-2 rounded-2xl border border-slate-150">
                      ...
                      <div className="relative min-w-[180px] sm:max-w-xs w-full">
                        ...
                        <input
                          ...
                          placeholder="কোর্স বা পরীক্ষার নাম খ্এজ্ন..."
                  ...
                  <div className="flex flex-col gap-3 animate-fade-in">
                        ...
                        <div className="flex items-start gap-2.5">
                          ...
                          <div>
                            ...
                            <p className="text-amber-800 text-[11px] font-medium mt-0.5">
                              আপনি রুটিন ও সিলেবাস দেখতে পারবেন। তবে "পরীক্ষার প্রস্তুতি", "Demo exam" এবং "Live exam"-এ অংশগ্রহণ করতে কোর্সে এনরোল করুন।
                            </p>
                          </div>
                        </div>
                        {itemBatch.courseId && (
                    ...
                    <div className="bg-white border border-indigo-100 py-2.5 px-1.5 sm:px-4 sm:py-4 rounded-2xl shadow-2xs space-y-2.5 sm:space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        {/* User Requested Button: "পরীক্ষার  প্রস্তুতি" */}
                        <button
                          type="button"
              ...
              <div className="bg-white border border-gray-100 p-5 rounded-3xl shadow-sm flex flex-col gap-4 text-xs">
                ...
                <div className="bg-amber-50 border border-amber-100/70 p-4 rounded-2xl flex flex-col gap-2">
                  ...
                  <p className="text-[11px] text-amber-700 leading-normal">
                    প্রশ্নে ভুল রিপোর্ট করে অথবা কাস্টম ব্যাখ্যা দিয়ে আপনি কন্ট্রিবিউটর পয়েন্ট অর্জন করতে পারেন। প্রতিটি অন্মোদিত কন্ট্রিবিউশন-এর জন্য ১টি পয়েন্ট দেওয়া হয়।
                  </p>
                  <div className="grid grid-cols-2 gap-3 text-center mt-1">
                    <div className="bg-emerald-50 border border-emerald-150 p-3 rounded-xl">
                      <span className="block text-[9px] text-emerald-600 font-extrabold">অন্মোদিত পয়েন্ট (Approved)</span>
      ...
      {!quizActive && !readerModeActive && (
        <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200/90 shadow-lg px-2 py-1 sm:py-1.5 flex items-center justify-around max-w-lg mx-auto md:max-w-2xl lg:max-w-4xl transition-all">
          {[
            { id: 'dashboard', label: 'হোম', icon: Home },
            { id: 'exams', label: 'লাইভ এক্সাম', icon: Award },
            { id: 'results', label: 'ফলাফল', icon: TrendingUp },
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center py-1 px-2.5 sm:px-3 rounded-xl transition-all duration-150 cursor-pointer min-w-[56px] ${
                  isActive
                    ? 'text-indigo-600 font-extrabold scale-105'
                    : 'text-slate-500 hover:text-slate-800 font-semibold hover:bg-slate-50'
                }`}
              >
                <div className={`p-1 rounded-lg transition-colors ${isActive ? 'bg-indigo-50 text-indigo-600' : ''}`}>
                  <Icon className={`w-5 h-5 sm:w-5.5 sm:h-5.5 ${isActive ? 'stroke-[2.5]' : 'stroke-[1.75]'}`} />
                </div>
                <span className={`text-[10px] sm:text-[11px] leading-tight tracking-tight mt-0.5 whitespace-nowrap ${isActive ? 'font-black text-indigo-700' : 'font-medium text-slate-600'}`}>
                  {tab.label}
                </span>
              </button>
            );
      ...
      {setupModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-3 sm:p-4 backdrop-blur-xs animate-fade-in">
          <div className="bg-white w-full max-w-[480px] max-h-[92vh] overflow-y-auto rounded-3xl p-5 sm:p-6 flex flex-col gap-4 shadow-2xl animate-scale-up">
              ...
              <div className="bg-slate-50/80 p-3.5 rounded-2xl border border-slate-200/80 flex flex-col gap-3">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-600 mb-1">
                    ১.১ মূল বিষয়/ক্যাটাগরি সিলেক্ট করুন:
                  </label>
                  <select
                ...
                <div>
                  ...
                  >
                    <option value="ALL">সকল উপ-অধ্যায়</option>
                ...
                {customExamSubSubcategoryOptions.length > 0 && (
                  <div>
                    <label className="block text-[11px] font-bold text-slate-600 mb-1">
                      ১.৩ অনুচ্ছেদ/টপিক সিলেক্ট করুন:
                    </label>
                    <select
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-600 mb-1">
                ২. পরীক্ষার প্রশ্নসংখ্যা নির্বাচন করুন:
              </label>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  { value: 999, label: 'আনলিমিটেড' },
                  { value: 10, label: '১০ মিনিট' },
                  { value: 20, label: '২০ মিনিট' }
                ].map(opt => (
                  <button
                    key={opt.value}
                    type="button"
